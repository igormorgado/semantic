...
class nlp:
    ...
    def lang_sent(self, text):
        ...
        for sen in sentences:
            sent_senti.append([sen.string,
                               round(sen.sentiment.polarity,2),
                               round(sen.sentiment.subjectivity,2)])
    ...
    def tokenize(self, text):
        ...
        words =  list(map(lambda word: word.lower(), WordPunctTokenizer().tokenize(text)))
        #print(len(words))
        ...
        #print(len(words))
        #words =(list(map(lambda token: snow.stem(token),words)))
